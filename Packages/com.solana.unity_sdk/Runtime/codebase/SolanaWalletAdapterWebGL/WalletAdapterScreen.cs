...
namespace Solana.Unity.SDK
{
    public class WalletAdapterScreen : MonoBehaviour
    {
        ...
        {
            // Loop through the wallets and filter only "Phantom" and "Solflare"
            foreach (var wallet in SolanaWalletAdapterWebGL.Wallets)
            {
                if ((wallet.name.Equals("Phantom", StringComparison.OrdinalIgnoreCase) ||
                     wallet.name.Equals("Solflare", StringComparison.OrdinalIgnoreCase)) &&
                     !_addedWallets.Contains(wallet.name))
                {

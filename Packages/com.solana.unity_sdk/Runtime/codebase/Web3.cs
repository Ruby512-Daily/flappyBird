...
namespace Solana.Unity.SDK
{
    ...
    public class Web3 : MonoBehaviour
    {
        ...
        public void CheckAndLoginOrLogout()
        {
            if (Wallet != null)
            {
                // Wallet is connected, call Logout
                Logout();
                walletBtn.GetComponentInChildren<TextMeshProUGUI>().text = "Connect Wallet";
            }
            else
            {
                // Wallet is not connected, call LoginWithWalletAdapter
                LoginWalletAdapter().AsUniTask().Forget();
            }
        }
